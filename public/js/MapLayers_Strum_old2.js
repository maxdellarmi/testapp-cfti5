...
function parseQuakeFile(INGVquakes){
    ...
    if (lines.length>1){
        ...
        for (var i=1; i<lines.length-1; i++){
    		...
    		var OnClickTextEN = '<div class="IWSTRUM"><div id="IWcloseSTRUM"><a onclick="infowindow2.close()" href="#"><img src="images/close.png" width="10px"></a></div>' + titleEN + '<br>' + '<br><a href="http://cnt.rm.ingv.it/en/event/' + StrumQuakeCode + '" target="_blank"> INGV-CNT page </a><br></div>'

    		var OnClickTextIT = '<div class="IWSTRUM"><div id="IWcloseSTRUM"><a onclick="infowindow2.close()" href="#"><img src="images/close.png" width="10px"></a></div>' + titleIT + '<br>' + '<br><a href="http://cnt.rm.ingv.it/event/' + StrumQuakeCode + '" target="_blank"> Pagina INGV-CNT </a><br></div>'
...
function HSITrequest(HSITcode, s){
    ...
    // read text from URL location
    var request = new XMLHttpRequest();
    request.open('GET', 'http://www.haisentitoilterremoto.it/rep/events/' + HSITcode.substring(0,1) + '/' + HSITcode.substring(1,2) + '/' + HSITcode + '/' + HSITcode + '_mcs.txt', true);
...
function openPopupSTRUM(marker, textEN, textIT, s, StrumQuakeCode){
	google.maps.event.addListener(marker, 'click', function() {
        ...
        if (flagHSIT[s] == 1){
           ...
           HSITlinkEN = '<br><abbr title=""><a href="http://eventi.haisentitoilterremoto.it/' + StrumQuakeCode + '/index.html " target="_blank"> HSIT page </a></abbr>'
           HSITlinkIT = '<br><abbr title=""><a href="http://eventi.haisentitoilterremoto.it/' + StrumQuakeCode + '/index.html " target="_blank"> Pagina HSIT </a></abbr>'
        }
