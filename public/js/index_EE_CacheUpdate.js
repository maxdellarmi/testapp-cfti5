...
function requestEEdata(){
    ...
    $.ajax({
        ...
        //         console.error(errorThrown);
        //     }
    console.log('loadJSONIndexEEdataFullCachedZIP (placeholder) -- NON ESEGUITO IN CASO DI GENERAZIONE NUOVA CACHE ALTRIMENTI SCRIVE DATI DOPPI....');
        // }).then( function( dummy ) {
	...
	for (var i = 0; i < EEmarkersArray.length; i++) {  // loop thorough each selected EE
		...
		var row = '';
                ...
                //  "TODO. NB. quakesTitleLOC contiene url da sostituire con quella desiderata
                //  <tr><td class="dateIW">133 B. C. - -</td><td class="timeIW">-</td><td class="meIW">Me: 0</td><td class="IoIW">Io: 0</td><td class="locationIW">Luni (SP)</td><td class="linkIW"><abbr class= "quakePageLink" title= "Link alla pagina del terremoto"> <a href="http://laravel.test/quake.php?50013IT" target="_blank"><img src="images/link2.png" width= "10" vertical-align="-30px"/></a></abbr></td></tr>"
                //pippo.MquakesTitleLOC[0][1].map(x => x.replaceAll("localhost","pippopippo"));

                //SOSTITUZIONE DELLA CHIAVE URL CON QUANTO DESIDERATO localhost
                saveJSONIndexEE.MquakesTitleLOC.push([i, quakesTitleLOC.map(x => x.replaceAll("laravel.test",WEBSITEROOT))]);
                ...
                ExportKmlR = ExportKmlR + "<b>" +  class_titleEE_EN[class_codeEE.indexOf(RecEE['EE_codeff'])] + "</b> <br> (" + RecEE['EEtypeExp'] + ")<br><br>Locality: <b>" + RecEE['EE_loc'] + "</b><br>Latitude: <b>" + RecEE['EE_Lat'] + "</b> <br>Longitude: <b>" + RecEE['EE_Lon'] + "</b><br><b><a href=" + virg + "https://cfti.ingv.it:8003/locality.php?" + RecEE['EE_nloc'] + "EN" + virg + ">Locality page </a></b><br> <br><br>" + CarRet + "<table>" + CarRet + "<tr> <td><b>Date </b></td><td><b>Time </b></td> <td><b>Io</b></td> <td><b>Me</b></td><td><b>Epicentral Area</b></td> </tr>" + CarRet + "<tr><td>"+ RecEE['EQdate'] + "</td> <td>" + RecEE['EQtime'] + "</td> <td>" + RecEE['EQIo'] + "</td> <td>" + RecEE['EQMe'] + "</td> <td>" + RecEE['EEepAreaExp'] + "</td></tr>" + CarRet + "</table>" + CarRet
            ...
            // SALVATAGGIO DELLE VARIABILI CREAZIONE ARRAY GLOBAL x sostituire l'elaborazione del ciclo while con valori costanti.
            // FUNZIONA SOLO PER CARICAMENTO INIZIALE
               $.ajax({
                   url: '/saveJSONFile?' + filenameToProcess,  //http://localhost/saveJSONFile?Filename => Route::post('/saveJSONFile', 'PhotoController@saveJSONFile');
                   type: 'POST',
                   dataType: 'json',
                   contentType: 'json',
...
/**
 * disabilita la dropdowns
 */
function outputFinisheDisableSelect() {
    $(document).ready(function() {
        try {
            document.getElementsByName("access")[0].disabled=true;
        } catch (e) {
...
function openPopupEE(marker, text, Nloc, E1textIT, E1textEN){
	...
	// specify language of popup window
		...
		marker.ContentPopupText = text.replaceAll("laravel.test", "localhost"); //Testo popup con logica applicata da visualizzare nella mappa
